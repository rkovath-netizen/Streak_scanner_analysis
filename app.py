...
if uploaded_files and st.button("🚀 Run Options Backtest"):
    ...
    # --- ON-SCREEN UI LOGGER ---
    st.markdown("### 🛠️ Live Execution Logs")
    log_container = st.empty()
    ui_logs = []
    
    def ui_log(msg):
        ui_logs.append(msg)
        # Keep only last 15 logs to prevent UI lag
        log_container.code("\n".join(ui_logs[-15:]), language="text")
    ...
    try:
        ...
        if trades_df.empty:
            st.error("⚠️ No trades executed. Look at the Live Execution Logs above to see what failed.")
        else:
